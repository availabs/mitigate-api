	...
	dbservice.promise(getData).then(d=> {
		...
		d.forEach(building => {
			...
			//console.log(building)
			 ...
			 let update = `update irvs.enhanced_building_risk
			 	 set building_type = '${building.building_type}', 
			     basement =  ${building.basement},
			     num_units =  ${building.num_units}

			 where building_id = ${building.id}`

			updates.push(dbservice.promise(update))
			// console.log('what is this', dbservice.asyncQuery)
			
			// console.log('updated', update, updateStatus)


		})
		//console.log()
		Promise.all(updates).then(finishUpdates => {
			console.log('finished',finishUpdates)
			dbservice.end();
		})

		// d is completed date

		// console.log('inserted', updates)
